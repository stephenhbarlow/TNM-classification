...
from data.datasets import ReportDataset, MultiLabelDataset
...
class BertDataLoader(DataLoader):
    ...
    def __init__(self, args, split, shuffle, drop_last=False):
        ...
        if self.split == 'train':
            self.dataset = ReportDataset(self.args.train_data_dir, self.tokenizer, self.max_len, self.args)
        elif self.split == 'val':
            self.dataset = ReportDataset(self.args.val_data_dir, self.tokenizer, self.max_len, self.args)
        else:
            self.dataset = ReportDataset(self.args.test_data_dir, self.tokenizer, self.max_len, self.args)
